...
# Create index with Lucene engine k-NN field
...
echo -e "\n\n3. Testing regular k-NN search (k=3)..."
curl -X GET "http://localhost:9200/test-knn-lucene/_search?preference=_only_node:opensearch-node2" \
  -H 'Content-Type: application/json' \
  -d '{
    "size": 5,
    "query": {
...
echo -e "\n\n4. Testing radial search with min_score=0.8..."
curl -X GET "http://localhost:9200/test-knn-lucene/_search?preference=_only_node:opensearch-node2" \
  -H 'Content-Type: application/json' \
  -d '{
    "size": 5,
...
echo -e "\n\n5. Testing radial search with max_distance=0.5..."
curl -X GET "http://localhost:9200/test-knn-lucene/_search?preference=_only_node:opensearch-node2" \
  -H 'Content-Type: application/json' \
  -d '{
    "size": 5,
