...
public class SemanticHighlightActionFilter implements ActionFilter {
    ...
    private static final java.util.concurrent.atomic.AtomicLong TOTAL_REQUESTS = new java.util.concurrent.atomic.AtomicLong();
    private static final java.util.concurrent.atomic.AtomicLong TOTAL_PROCESSING_TIME = new java.util.concurrent.atomic.AtomicLong();
    private static final java.util.concurrent.atomic.AtomicLong TOTAL_DOCUMENTS_PROCESSED = new java.util.concurrent.atomic.AtomicLong();
            ...
            chain.proceed(task, SearchAction.NAME, plainRequest, ActionListener.wrap((SearchResponse baseResponse) -> {
                ...
                enhanceWithSemanticHighlights(baseResponse, originalRequest, startTime).whenComplete((enhancedResponse, error) -> {
                    ...
                    if (error != null) {
                        ...
                        TOTAL_DOCUMENTS_PROCESSED.addAndGet(enhancedResponse.getHits().getHits().length);
    ...
    private CompletableFuture<List<HighlightResult>> executeBatchSemanticHighlighting(SearchHit[] hits, SemanticHighlightConfig config) {
        ...
        return CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).thenApply(v -> {
            ...
            log.info(
                "[semantic-hl-filter] Batch highlighting completed: total={}ms, documents={}, avgMLTime={}ms, SUCCESS_COUNT={}",
                batchDuration,
                results.size(),
                avgMLTime,
                successCount
    ...
    public static String getPerformanceStats() {
        ...
        long totalTime = TOTAL_PROCESSING_TIME.get();
        long totalDocs = TOTAL_DOCUMENTS_PROCESSED.get();

        return String.format(
            "SemanticHighlightActionFilter Stats: requests=%d, totalTime=%dms, documents=%d, avgTime=%.1fms, avgDocsPerRequest=%.1f",
            totalRequests,
            totalTime,
            totalDocs,
            totalRequests > 0 ? (double) totalTime / totalRequests : 0.0,
            totalRequests > 0 ? (double) totalDocs / totalRequests : 0.0
        );
    }
}
