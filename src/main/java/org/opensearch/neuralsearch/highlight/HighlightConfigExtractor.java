...
public class HighlightConfigExtractor {
     ...
     * @return extracted configuration (may be invalid)
     */
    public HighlightConfig extract(SearchRequest request, SearchResponse response) {
        try {
            if (request == null || request.source() == null) {
                return HighlightConfig.invalid("Search request or source is null");
            }

            HighlightBuilder highlighter = request.source().highlighter();
            if (highlighter == null) {
                return HighlightConfig.invalid("No highlighter found in request");
            }

            String fieldName = extractSemanticField(highlighter);
            if (fieldName == null || fieldName.isEmpty()) {
                return HighlightConfig.invalid("No semantic highlight field found");
            }

            String modelId = extractModelId(highlighter);
            if (modelId == null || modelId.isEmpty()) {
                return HighlightConfig.invalid("Model ID is required");
            }

            String queryText = extractQueryText(request);
            if (queryText == null || queryText.isEmpty()) {
                return HighlightConfig.invalid("Query text is required");
            }
